 ...
 */
public class PhotoListType extends ASingleContentType{
	protected final Logger logger = Logger.getLogger(getClass());
    /** string constant that represents type for this page */
    public static final String TYPE="system_photo_list";
	/** rus type */
			...
			throws Exception
	{
		...
		if (user!=null&&Utils.isUserInRole(user, "user")){
			...
			hs.put(gallery.web.controller.pages.types.PhotoDeleteType.TYPE, new EmptySubmodule());
			hs.put(gallery.web.controller.pages.types.PhotoEditType.TYPE, new EmptySubmodule());
			hs.put(gallery.web.controller.pages.types.PhotoAddType.TYPE, new EmptySubmodule());
			url.setSubmodules(hs);
			request.setAttribute(PHOTOS_ATTRIBUTE, photoService.getShortByPropertyValueOrdered(SELECT_FIELDS,"id_users",user.getId(),ORDER_BY,ORDER_HOW));
		}else{
